  ...
  return (
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
      {projects.map((project) => {
        ...
        const availableCount = projectTickets.filter((t) =>
        ...
        return (
          <Card
            ...
            className={`ui-panel theme-panel min-w-0 overflow-hidden rounded-none border shadow-none ${
              highlighted
                ? "border-accent/50 ring-1 ring-accent/30"
                : "border-[var(--hairline)] hover:border-accent/30"
            }`}
          >
            <Card.Header className="flex min-w-0 items-start gap-3 overflow-hidden p-5 pb-3">
            ...
            <Card.Content className="space-y-3 px-5 pb-4">
              ...
              {projectTickets.length === 0 ? (
                ...
                <div className="flex flex-col gap-1.5">
                  {projectTickets.map((t, i) => (
                    <div
                      ...
                      className="stagger-item theme-panel-strong flex min-w-0 items-center justify-between gap-3 border border-[var(--hairline)] px-3 py-2.5 text-sm"
                    >
                      <span className="theme-ink min-w-0 flex-1 truncate pr-2" title={t.name}>
                        {t.name}
                      </span>
                  ...
                  <Button
                    ...
                    onPress={() =>
                      setExpanded((prev) => ({ ...prev, [project.id]: false }))
                    }
                  >
                    收起
                  </Button>
                ...
                <Button
                  size="sm"
                  variant="secondary"
                  className="w-full rounded-none border border-[var(--hairline)]"
                  onPress={() =>
