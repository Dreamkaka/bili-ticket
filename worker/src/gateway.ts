...
function baseUrl(env: Env): string {
  // 兼容 wrangler --var / .dev.vars / [vars] 注入
  const raw =
    env.GATEWAY_HTTP_URL ||
    (env as Env & { GATEWAY_URL?: string }).GATEWAY_URL ||
    "";
...
export async function loadTargets(env: Env): Promise<{
  targets: TargetsCache | null;
  refreshed: boolean;
  error?: string;
}> {
  const ttlSec = Math.max(60, Number(env.TARGETS_TTL_SEC || 3600) || 3600);
  const cached = await env.PROBE_STATE.get<TargetsCache>(TARGETS_KEY, "json");
  const now = Date.now();
  ...
  try {
    ...
    const core_ids = Array.isArray(body.core_ids) ? body.core_ids.map(String) : [];
    ...
    await env.PROBE_STATE.put(TARGETS_KEY, JSON.stringify(next));
...
export async function nextRunCounter(env: Env): Promise<number> {
  const raw = await env.PROBE_STATE.get(RUN_COUNTER_KEY);
  const n = (Number(raw || 0) || 0) + 1;
  await env.PROBE_STATE.put(RUN_COUNTER_KEY, String(n));
  return n;
}
